...
import type { TodosResolvedResponse, TodosRejectedResponse } from './todos';
...
function TodosStore(props: TodosPropTypes) {
  ...
  async function asyncGetTodos() {
    ...
    try {
      const response = await httpGetRequest<TodosResolvedResponse>({
        urlSuffix: 'todos',
      });

      dispatch({
        type: TODOS_TYPES.GET_TODOS_SUCCESSFUL,
        payload: {
          data: response.data,
        },
      });
      ...
      dispatch({
        type: TODOS_TYPES.GET_TODOS_FAILED,
        payload: {
          error:
            (request.isAxiosError(error) && (error.response?.data as TodosRejectedResponse)?.message) ||
            GENERIC_ERROR_MSG,
        },
      });
    }
